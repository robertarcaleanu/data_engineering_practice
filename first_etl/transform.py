...
class Transformer:
    def __init__(self) -> None:
        self.COLUMNS = ['icao24', 'departure_datetime', 'departure_airport', 'arrival_datetime', 'arrival_aiport']
    ...
    def _calculate_flight_duration(self, df: pl.DataFrame) -> pl.DataFrame:
        df = df.with_columns(
            (pl.col("arrival_datetime") - pl.col("departure_datetime")).dt.total_minutes().alias("flight_duration_in_minutes")
        )
        return df
